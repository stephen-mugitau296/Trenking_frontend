...
const About = () => {
  ...
  async function search(key){
    ...
    let result = await fetch('http://127.0.0.1:8000/api/user_search/'+key);
        ...
        return(
          <div key={item.id}>
            <div className="border-2 border-gray-2 rounded">
              <Link to={`/shops/${item.merchant}`}>
              <img src={`http://127.0.0.1:8000/${item.logos}`} alt={item.merchant} className='w-full h-[300px] object-cover' />
              </Link>
              <Link to ={`/shops/${item.merchant}`}>
              <h2>{item.merchant}</h2>
              </Link>
  ...
  return (
    <div className='mt-32'>
      ...
      <h1 className='mb-2 text-xl font-bold'>Search your prefered shop</h1>
