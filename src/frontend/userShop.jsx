...
const UserShop = () => {
  ...
  const handleNav = () => {
    ...
    useEffect(()=>{
        ...
        axios.get(`/api/getproducts/${merchant}`).then(res =>{
            if(isMounted)
                {
        ...
        axios.get(`/api/fetchCategory/${merchant}`).then(res=>{
    ...
    async function search(key){
      ...
      let result = await fetch(`http://127.0.0.1:8000/api/user-search/${merchant}/`+key);
    ...
    if(loading)
        {
          ...
          showCategory = userCategory.map((item) =>{
            return(
              <SwiperSlide key={item.id}>
                <div className="mx-12">
                  <Link to={`/collection/${item.slug}/${item.merchant}`}>
        ...
        if(productcount){
          productList = currentProduct.map((item) =>{
            return(
              <div key={item.id}>
                <div className="border-2 border-gray-2 rounded">
                <Link to={`/shops/${item.category.slug}/${item.merchant}/${item.id}`}>
          ...
          productList =
          <div>
            <h2>No product available for {category.merchant}</h2>
          </div>
...
<div className='flex flex-col justify-center mt-12'>
      ...
      <div className='flex justify-between bg-gray-100 m-8 p-2'>
        ...
        <div>
        <button  className="flex gap-2 justify-center items-center text-xl right-0 z-10 md:hidden" >
          Category <FaChevronDown /></button>
        </div>
        </div>
